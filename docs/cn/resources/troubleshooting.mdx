...
<AccordionGroup>
  ...
  <Accordion title="`--compute cpu` 或 `--compute gpu` 在 Qualcomm AI Hub 模型上报错">
    Qualcomm AI Engine Direct 仅支持 NPU。请使用 `--compute npu`（或省略该标志——`npu` 是 `qairt` 的默认值）。如需在 CPU/GPU 上运行，改用 [llama.cpp 运行时](/cn/get-started/platforms#llamacpp)的 GGUF 模型。
  </Accordion>
</AccordionGroup>
