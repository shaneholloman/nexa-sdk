...
<AccordionGroup>
  ...
  <Accordion title="`--device cpu` or `--device gpu` errors with a Qualcomm AI Hub Model">
    Qualcomm AI Engine Direct is NPU-only. Use `--device npu` (or omit the flag — `npu` is the default for `qairt`). To run on CPU/GPU, switch to a GGUF model on the [llama.cpp runtime](/en/get-started/platforms#llamacpp).
  </Accordion>
</AccordionGroup>
