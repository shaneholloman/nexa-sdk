...
<Tip>`Q4_0` has the best Hexagon NPU support. See [Precisions (Quantizations) & device placement](/en/models/supported#precisions-quantizations--device-placement).</Tip>
...
<Warning>Qualcomm AI Hub Models run on NPU only. Using `--device cpu` or `--device gpu` returns an error.</Warning>
